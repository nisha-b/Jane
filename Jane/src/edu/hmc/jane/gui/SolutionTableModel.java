...
public class SolutionTableModel extends AbstractTableModel {
    ...
    private String addDecimal(int cost, int multiple) {
        String strCost = Integer.toString(cost);
        StringBuilder sb = new StringBuilder(strCost);
        if (multiple != 0) {
            sb.insert(strCost.length() - multiple, ".");
        }
        return sb.toString();   
    }
    ...
    public Object getValueAt(int x, int y) {
        ...
        if (compress) {
            switch (y) {
                ...
                */
                case 6:
                    return compressedSolns.get(row).get(subrow).cost/ eventSolvers.get(x).getDecimalPlaces();
            }
        } else {
            switch (y) {       
                case 0:
                ...
                */
                case 6:
                    return addDecimal(eventSolvers.get(x).cost, eventSolvers.get(x).getDecimalPlaces());
                    //return eventSolvers.get(x).cost/ eventSolvers.get(x).getMultiplier();
            }
        }
        return -1;
    }
